function DecImg=Decryption(EncImg,ExternalKey,H1)
    N=size(EncImg,1);
    ...
    RHVDC=zeros(N);
    for i=1:N
        RHVDC(LatinPer(:,i),i)=bitxor(EncImg(:,i),LatinKey(LatinPer(:,i),i));   
    end
    ...
    for i=1:N
        RVDC(LatinPer(:,i),i)=bitxor(RHVDC(:,i),LatinKey(LatinPer(:,i),i));
    end
